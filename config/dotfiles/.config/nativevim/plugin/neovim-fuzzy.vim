...
" Version:      0.3
"
" Modified to have separate command for buffers and files
"
...
command! -nargs=? FuzzyFiles             call s:fuzzy_open(0, 1, <q-args>)
command! -nargs=? FuzzyBuffers           call s:fuzzy_open(1, 0, <q-args>)
command! -nargs=? FuzzyAll               call s:fuzzy_open(1, 1, <q-args>)
...
" NOTE: i added toggles for options
function! s:fuzzy_open(show_bufs, show_files, root) abort
  ...
  if a:show_bufs
    " Get open buffers without current and alternative (alt should be first)
    let bufs = filter(range(1, bufnr('$')),
        \ 'buflisted(v:val) && !empty(bufname(v:val)) && v:val != bufnr("#") && v:val != bufnr("%")')
  ...
  " Get all files, minus the ignored buffers.
  if a:show_files == 1
    try
